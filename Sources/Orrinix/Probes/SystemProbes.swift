...
struct DockerProbe: StorageProbe {
    func probe() async -> [StorageItem] {
        let container = URL.home("Library/Containers/com.docker.docker")
        let vms = container.appending(path: "Data/vms")
        ...
        guard let docker = Shell.which("docker"),
            ...
            return [await ProbeSupport.directoryItem(
                id: "docker-data", category: .docker, name: "Docker Desktop data",
                detail: "Docker is not running. Delete to remove its local images, containers, virtual disk, and settings.",
                url: container, safety: .review,
                action: .removePaths([container]),
...
struct SystemProbe: StorageProbe {
    func probe() async -> [StorageItem] {
        ...
        if let item = await ProbeSupport.directoryItem(
            ...
            detail: "/Library/Caches. Regenerated by the daemons that own them. SIP-protected Apple caches are skipped.",
            url: systemCaches, safety: .safe,
            action: .emptyDirectories([systemCaches]), minimumBytes: 10 * ProbeSupport.megabyte
        ) {
            items.append(item)
        }
