...
#
# Needs: gh (logged in), a notarytool keychain profile, and repositories that
# belong to the current maintainer. Set these explicitly before releasing.
set -euo pipefail

root=$(cd "$(dirname "$0")/.." && pwd)
part=${1:-patch}
repo=${GITHUB_REPOSITORY:?Set GITHUB_REPOSITORY to your GitHub repository, e.g. Abbasm234/orrinix}
tap_repo=${HOMEBREW_TAP_REPOSITORY:?Set HOMEBREW_TAP_REPOSITORY to your Homebrew tap repository}
cask=${HOMEBREW_CASK_NAME:-orrinix}
export NOTARY_PROFILE=${NOTARY_PROFILE:?Set NOTARY_PROFILE to your notarytool keychain profile}
